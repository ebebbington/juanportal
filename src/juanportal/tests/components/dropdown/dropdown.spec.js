...
test("It renders the data passed in", () => {
  const dataToPopulateULWith = [{text: 'Simon', checked: true, id: 1003}, { text: "James", checked: false}]
  const title = "Friends"
...
test("It calls the change handler if passed in, and updates the selected text", () => {
  ...
  render(<Dropdown title={title} liData={dataToPopulateULWith} checkedHandler={handler} />);
  const simonCheckbox = document.querySelector('input[data-id="1003"]')
  simonCheckbox.click()
  expect(passedBackData).toStrictEqual({"checked": false, "id": "1003", "text": "Simon"})
});