...
test("Passed in a handler gets called when button pressed", () => {
  ...
  render(<LiveEditInput title={title} inputVal={inputValue} saveHandler={handler} />);
  const button = document.querySelector("button")
  button.click()
...
test("Id is set when passed in, and passed into the handler", () => {
  ...
  render(<LiveEditInput title={title} inputVal={inputValue} id={id} saveHandler={handler} />);
  const button = document.querySelector("button")
