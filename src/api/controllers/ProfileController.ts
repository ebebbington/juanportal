...
export default class ProfileController { // eslint-disable-line allow class to only have static props
  /**
  ...
  public static async GetProfilesByAmount (req: express.Request<import('express-serve-static-core').ParamsDictionary>, res: express.Response, next: Function): Promise<any> {
    ...
    logger.info(`Profiles with a length of ${(profiles as any[]).length} were found`)
    const data: IData = {
      success: true,
